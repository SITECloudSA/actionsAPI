...
const generateJSON = ({ files, overrideDocs, path }) => {
  Object.keys(files).forEach((tag) => {
    files[tag].getRoutes().forEach((r) => {
      const url = `${path}${tag}${r.path}`;
  ...
  swaggerConfig = overrideDocs ? overrideDocs(swaggerConfig, files, path) : swaggerConfig;
...
`;
module.exports = ({ path, files, overrideDocs }) => {
  return (req, res) => {
    if (req.url === "/config.json") {
      res.send(generateJSON({ files, overrideDocs, path }));
    } else {
