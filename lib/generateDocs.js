...
const generateJSON = ({ prefix, files, overrideDocs, path }) => {
  ...
  swaggerConfig = overrideDocs ? overrideDocs(swaggerConfig, files, prefix, path) : swaggerConfig;
...
`;
module.exports = ({ path, prefix, files, overrideDocs }) => {
  return (req, res) => {
    if (req.url === "/config.json") {
      const computedPrefix = prefix || req.originalUrl.split("/").slice(0, -2).join("/");
      res.send(generateJSON({ prefix: computedPrefix, files, overrideDocs, path }));
    } else {
