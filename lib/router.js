...
module.exports.apiApp = () => {
  ...
  apiApp.generateDocs = ({ path = "/docs", ...configs } = {}) => apiApp.OrginalUse(path, generateDocs({ files, path, ...configs }));
  apiApp.generateSDK = ({ path = "/ws", disableSocket, prefix, ...config } = {}) => {
    !disableSocket && apiApp.OrginalUse(path, sockethandler({ prefix: prefix || apiApp.prefix, path }));
